...
exports.newMedia = async function(req, res) {
    if (req.session.userId)
    {
        ...
        if(ownerID == req.session.userID) //Check that logged in user owns the showcase which media is being added to
        {
            ...
            var sql = "SELECT * FROM Showcase WHERE idShowcase = ?";
            conn.query(sql, showcaseID, function (err, results) {
                if (err) throw err;
                if(results.length > 0) //Showcase exists
                {
                    if(results[0].idUser == req.session.userID) // Showcase belongs to user, insert new media
                    {
                        sql = "INSERT INTO Media (idShowcase, content) VALUES ('"+showcaseID+"','"+URL+"')";
                        conn.query(sql, function (err, results) {
                            if (err) throw err;
                    ...
                    {
                        res.status(401).send('Showcase does not belong to user');
                        return;                    }
                }
                else
                {
                    res.status(401).send('Showcase does not exist');
                    return;
                }
            });
...
exports.retrieveMedia = async function(req, res) {
    ...
    conn.query(sql, [mediaid], function (err, results) {
        ...
        {
            res.status(401).send('Media does not exist');
        }
    });
    }
